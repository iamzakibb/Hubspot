# single-subtheme fetch & push script
# set your system access token and choose one subtheme to process
$systemAccessToken = ''

# Base theme folder name in HubSpot
$baseThemeName = 'Code Ninja - Marketing Website'

# <-- SET ONLY THIS VARIABLE to the one subtheme you want to process -->
$subTheme = 'CNMW - DEV'    # e.g. 'CNMW - DEV' or 'CNMW - PRODUCTION' or 'CNMW - QA'
# -------------------------------------------------------------------------
...
# Build the HubSpot theme path and branch/folder names for this single subtheme
...
$folderSafe = $safeName -replace '-', '_'   # underscores for filesystem clarity
...
# 2.a) Ensure origin points at the tokenized repo URL and check for remote branch
& git remote set-url origin $repoUrl 2>&1 | Out-Null
$ls = & git ls-remote --heads origin $branchName 2>&1 | Out-String
if ($ls.Trim()) {
...
if (Test-Path $dest) {
    ...
    try { Remove-Item -Recurse -Force -LiteralPath $dest -ErrorAction Stop } catch { Write-Warning ("Could not clean {0}: {1}" -f $dest, $_.Exception.Message) }
}
...
# Ensure the dest exists and contains files — if not, log and stop
...
if (-not $fetchedFiles -or $fetchedFiles.Count -eq 0) {
    Write-Warning "No files were fetched into $dest; aborting commit for $branchName."
    exit 0
}
...
Write-Host "`n✅ Done for subtheme: $subTheme"
